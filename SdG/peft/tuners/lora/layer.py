...
class LoraLayer(BaseTunerLayer):
    ...
    def __init__(self, base_layer: nn.Module, **kwargs) -> None:
        ...
        self.index = 8
        self.lora_index = nn.ModuleDict({})
        self.lora_w_u = nn.ParameterDict({})
        self.lora_w_vt = nn.ParameterDict({})
    ...
    def update_layer(
        ...
        if lora_use_dash:
            ...
            self.lora_w_vt[adapter_name] = nn.Parameter(torch.zeros(self.index, self.in_features))
    ...
    def reset_lora_parameters(self, adapter_name, init_lora_weights):
        ...
        if adapter_name in self.lora_A.keys():
            ...
            if self.lora_use_dash[adapter_name]:
                nn.init.zeros_(self.lora_index[adapter_name].weight)
                nn.init.zeros_(self.lora_w_u[adapter_name])
                nn.init.zeros_(self.lora_w_vt[adapter_name])
        ...
        for i, active_adapter in enumerate(unique_adapters):
            ...
            if self.lora_use_dash[active_adapter]:
                if self.FLAG < self.warmup:
                    if self.FLAG == 0:
                        self.lora_index[active_adapter].requires_grad = False
                        self.lora_w_u[active_adapter].requires_grad = False
                        self.lora_w_vt[active_adapter].requires_grad = False
                    self.FLAG += 1
                
                elif self.FLAG == self.warmup:
                    delta_W = (lora_B.weight @ lora_A.weight) * scaling
                    delta_W = delta_W.to(dtype=torch_result_dtype)
                    weight_u, weight_sigma, weight_vt = torch.linalg.svd(self.base_layer.weight.to(dtype=torch.float32), full_matrices=False)
                    ...
                    self.lora_w_u[active_adapter] = nn.Parameter(weight_u[:, top_index].to(torch_result_dtype))
                    self.lora_w_vt[active_adapter] = nn.Parameter(weight_vt[top_index, :].to(torch_result_dtype))
                    self.lora_w_u[active_adapter].requires_grad = False
                    self.lora_w_vt[active_adapter].requires_grad = False

                    self.lora_index[active_adapter].requires_grad = True
                    self.FLAG += 1

                if self.FLAG > self.warmup:
                    lora_output += dropout(sub_batch) @ (self.lora_w_u[active_adapter] @ torch.diag(self.lora_index[active_adapter].weight.view(-1)) @ self.lora_w_vt[active_adapter]).T
...
class Linear(nn.Module, LoraLayer):
    ...
    def get_delta_weight(self, adapter) -> torch.Tensor:
        ...
        if self.lora_use_dash[adapter]:
            lora_index = self.lora_index[adapter].weight
            w_u = self.lora_w_u[adapter]
            w_vt = self.lora_w_vt[adapter]

        if cast_to_fp32:
            weight_A = weight_A.float()
            weight_B = weight_B.float()
            if self.lora_use_dash[adapter]:
                lora_index = lora_index.float()
                w_u = w_u.float()
                w_vt = w_vt.float()

        if self.lora_use_dash[adapter]:
            output_tensor = transpose(weight_B @ weight_A, self.fan_in_fan_out) * self.scaling[adapter] + \
                    w_u @ torch.diag(lora_index.view(-1)) @ w_vt
        else:
            output_tensor = transpose(weight_B @ weight_A, self.fan_in_fan_out) * self.scaling[adapter]
        ...
        if cast_to_fp32:
            ...
            if self.lora_use_dash[adapter]:
                self.lora_index[adapter].weight.data = lora_index.to(dtype)
                self.lora_w_u[adapter] = w_u.to(dtype)
                self.lora_w_vt[adapter] = w_vt.to(dtype)
            ...
            for active_adapter in self.active_adapters:
                ...
                if not self.use_dora[active_adapter]:
                    ...
                    if self.lora_use_dash[active_adapter]:
                        ...
                        if self.FLAG > self.warmup:
                            result += dropout(x) @ (self.lora_w_u[active_adapter] @ torch.diag(self.lora_index[active_adapter].weight.view(-1)) @ self.lora_w_vt[active_adapter]).T
                            return result
